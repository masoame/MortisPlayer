...
namespace SpiderVideo
{
    ...
    size_t SpiderTool::header_callback(char* buffer, size_t size, size_t nitems, void* userdata)
    {
        ...
        if (std::regex_match(str, result, getHeaderValue)) {
            _userdata->loadSize = std::atoi(result[1].str().c_str()); 
            _userdata->size = 0;
        }
        return size * nitems;
    }
    ...
    size_t SpiderTool::write_callback(void* contents, size_t size, size_t nmemb, void* userp)
    {
        ...
        if (_userdata->_type & saveBuffer)
            _userdata->buffer.insert(_userdata->buffer.end(), _backcalldata, _backcalldata + realsize);
    ...
    CURLcode SpiderTool::RequestInit(std::string_view request_url)
    {
        ...
        this->_pCurl.reset(curl_easy_init());
        if (this->_pCurl == nullptr) return CURLE_FAILED_INIT;
	...
	{
        CURLcode code = curl_easy_perform(this->_pCurl);
        if (code != CURLE_OK) return code;
    ...
    CURLcode SpiderTool::RequestOnlyHeader()
    {
        ...
        code = curl_easy_perform(this->_pCurl);
        if (code != CURLE_OK) return code;
