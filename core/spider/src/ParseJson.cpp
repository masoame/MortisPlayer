...
namespace ParseJson
{
	namespace BiliBili
	{
		...
		std::list<std::string> GetSuportedVideoCodecs(const::nlohmann::json& bilibili_json,const std::string& quality) noexcept
		{
			try
			{
	...
	std::optional<std::vector<std::string>> ParseYouTube(std::string&& YouTube_html_str)
	{
		...
		if ((std::regex_search(YouTube_html_str, videoResults, getVideoResource) == false)) 
			return {};
		if ((std::regex_search(YouTube_html_str, titleResults, getTitleResource) == false)) 
			return {};
